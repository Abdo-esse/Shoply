...
fetch(jsonFile)
  .then(response => response.json())  // Récupérer le JSON
  .then(data => {
    // Utilisation de la boucle 'for' pour itérer sur les produits
    for (let i = 0; i < data.length; i++) {
      const product = data[i]; // Récupérer l'objet produit
      const { id, title, price, description, image } = product;
      console.log(id, title, price, description, image);

      // Ajouter le contenu HTML dynamique pour chaque produit
      add.innerHTML += `
