...
//             <div
//               class="group bg-gray-100 w-10 h-10 flex items-center justify-center rounded-full cursor-pointer absolute top-4 right-4  z-50  hover:bg-red-600">
...
fetch('/assets/data/products.json')
  .then((response) => response.json())
  .then((data) => {
    myProducts = data; // Assign the fetched products data
    displayCategories(myProducts); // Display all categories initially
  })
  .catch((error) => console.error('Error fetching products:', error));

// Function to display categories (product cards)
function displayCategories(products) {
  ...
  products.forEach(product => {
    ...
    section.innerHTML += `
      <!-- product ${id} -->
      <div id="card-prod-${id}" class="bg-white rounded-2xl p-5 cursor-pointer relative hover:shadow">
        <div id="fav-btn-${id}" onclick="favBtn(this)" class="fav-btn-class group bg-gray-100 w-10 h-10 flex items-center justify-center rounded-full cursor-pointer absolute top-4 right-4 z-50 hover:bg-green-600">
          <svg xmlns="http://www.w3.org/2000/svg" width="16px" class="group-hover:fill-white inline-block" viewBox="0 0 64 64">
...
function filterByCategory(e) {
  if (e.target.value === "All" ||e.target.value === "catégoriem" ) {
    displayCategories(myProducts); // Show all products if 'All' is selected
  } else {
    const filteredProducts = myProducts.filter(product => product.catégorie === e.target.value);
    displayCategories(filteredProducts); // Show filtered products
  }
}

// Function to simulate adding product to cart (you can implement this function as needed)
